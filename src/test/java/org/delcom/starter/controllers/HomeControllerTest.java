...
class HomeControllerTest {
    ...
    void perolehanNilai_GradeE() {
        String input = String.join("\n", "10 15 10 15 20 30", "PA|100|20", "T|100|20", "K|100|20", "P|100|20", "UTS|100|20", "UAS|100|20", "---");
        String base64Input = encodeBase64(input);
        ResponseEntity<String> response = controller.perolehanNilai(base64Input);
        String result = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertTrue(result.contains(">> Nilai Akhir: 20.00"));
    ...
    @Test
    @DisplayName("palingTer - Single Number Input")
    void palingTer_SingleNumber() {
        String base64Input = encodeBase64("42");
        ResponseEntity<String> response = controller.palingTer(base64Input);
        String result = response.getBody();
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertTrue(result.contains("Tertinggi: 42"));
        assertTrue(result.contains("Terendah: 42"));
        assertTrue(result.contains("Terbanyak: 42 (1x)"));
        assertTrue(result.contains("Tersedikit: 42 (1x)"));
    }

    @Test
    @DisplayName("palingTer - All Same Numbers")
    void palingTer_AllSameNumbers() {
        String base64Input = encodeBase64("5 5 5 5 5");
        String expected = "Tidak ada angka unik";
        ResponseEntity<String> response = controller.palingTer(base64Input);
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(expected, response.getBody());
